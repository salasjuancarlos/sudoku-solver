...
static inline bool apply_naked_single(const uint64_t idx, sketch_notes_t &s, char *sudoku) noexcept {
...
static inline bool check_row(const uint64_t row, sketch_notes_t &s, char *sudoku) noexcept {
    ...
    const __uint128_t &row_idx_mask = row_to_cells_masks[row];
    ...
    while (available_bitmask) [[likely]] {
        ...
        if (!apply_naked_single(idx, s, sudoku)) [[unlikely]] return false;
    }
...
static inline bool check_col(const uint64_t col, sketch_notes_t &s, char *sudoku) noexcept {
    ...
    // Buscar y aplicar naked singles
    const __uint128_t &col_idx_mask = col_to_cells_masks[col];
...
static inline bool check_sqr(const uint64_t sqr, sketch_notes_t &s, char *sudoku) noexcept {
    ...
    // Buscar y aplicar naked singles
    const __uint128_t &sqr_idx_mask = sqr_to_cells_masks[sqr];
...
static inline bool find_solution(sketch_notes_t &s, char *sudoku) noexcept {
    ...
    while (free_copy) {
        ...
        const uint64_t pm = (s.rows_bitmask[rcs.row] & s.cols_bitmask[rcs.col] & s.sqrs_bitmask[rcs.sqr]);
...
static inline void solve(char *sudoku) noexcept {
    ...
    for (uint64_t i = 0; i < 9; ++i) {
        s.rows_bitmask[i] = (~s.rows_bitmask[i]) & 0x1FF;
        s.cols_bitmask[i] = (~s.cols_bitmask[i]) & 0x1FF;
        s.sqrs_bitmask[i] = (~s.sqrs_bitmask[i]) & 0x1FF;
    }
